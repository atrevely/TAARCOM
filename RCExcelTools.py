...
class PivotTables:
    ...
    def create_pivot_table(self, excel_file, data_sheet_name, pivot_sheet_name, row_fields, col_field,
                           data_field, page_field=None):
        ...
        try:
            ...
            piv_data_field = piv_table.AddDataField(piv_table.PivotFields(data_field),
                                                    f'Sum of {data_field}', self.win32c.xlSum)
            piv_data_field.NumberFormat = '$#,##0'
        except Exception:
            logger.error(f'Pivot table could not be created in file {excel_file}. Error: {traceback.format_exc(0)}')
...
def tab_save_prep(data):
    ...
    for col in mixed_cols:
        data[col] = data[col].map(lambda x: pd.to_numeric(x, errors='ignore'))
        data[col] = data[col].replace(to_replace=np.nan, value='')
